        ...
        if stimSets(ii)
            ...
            for iii = 1:length(stim)
                if isfield(neuron.Sounds.(soundCat), stim{iii})
                    ...
                    psthBinSlideM = mean(psthBinSlide, 2);
                    baselineHzM = mean(reshape(psthBinSlideM(windowBGSlide(1):windowBGSlide(2),:), numel(psthBinSlideM(windowBGSlide(1):windowBGSlide(2),:)), 1));
                    baselineHzSD = std(reshape(psthBinSlideM(windowBGSlide(1):windowBGSlide(2),:), numel(psthBinSlideM(windowBGSlide(1):windowBGSlide(2),:)), 1));
                    ...
                    norm_pop(populationcount, 1:length(psthBinSlideM)) = psthBinSlideM ./ baselineHzM;
                    sub_pop(populationcount, 1:length(psthBinSlideM)) = psthBinSlideM - baselineHzM;
                    ...
                    for j = 1:length(psthBinSlideM)
                        psthSlideHz_population{populationcount,3+j} = psthBinSlideM(j);
                    end
                    
                    %% SMI
                    clear psthSlideSMI
                    psthSlideSMI = log10((psthBinSlideM+C)/(baselineHzM+C));
                    ...
                    clear psthSlideRMI
                    if baselineHzM > 0
                        psthSlideRMI = (psthBinSlideM-baselineHzM)/(psthBinSlideM+baselineHzM);
                    else
                        psthSlideRMI = nan(length(psthBinSlideM));
                    end
                    ...
                    if baselineHzSD ~= 0 % Can't make a z score with a sd of 0, dummy
                        psthSlideZS = (psthBinSlideM - baselineHzM) / (baselineHzSD);
                    else
                        psthSlideZS = nan(size(psthBinSlideM));
                    end
